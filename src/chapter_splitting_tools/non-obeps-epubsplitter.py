...
def export_chapters(epub_dir, output_dir, manifest, spine, base_path):
    """Extracts and saves individual chapters as formatted plain text files with the chapter title at the top."""
    ...
    for i, idref in enumerate(spine):
        ...
        if os.path.exists(chapter_path):
            ...
            # Extract the chapter title from metadata: try <title> first, then <h1>
            ...
            # Convert <br> tags to newline so line breaks aren't lost
            ...
            # Prepend the chapter title (without indentation) followed by a blank line
            plain_text = chapter_title + "\n\n" + "\n\n".join(formatted_paragraphs)
