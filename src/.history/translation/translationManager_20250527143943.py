...
def run_proofing_phase(glossary, log_message, pause_event=None, cancel_flag=None, subphase=None):
    ...
    if subphase is None or subphase == "consistency" or subphase in ["gender", "final"]:
        # --- Consistency Proofing
        log_message("=== Subphase: Consistency Check ===")
        proofreader.check_consistency("output", pause_event, cancel_flag)
        
        if subphase == "consistency":
            return

    if subphase is None or subphase == "gender" or subphase == "final":
        # --- Gender Proofing
        log_message("=== Subphase: Gender Proofing ===")
        proofreader.check_gender_consistency("output", pause_event, cancel_flag)
    ...
    if subphase is None or subphase == "final":
        ...
        proofreader.proofread_all_files("output", pause_event, cancel_flag)
