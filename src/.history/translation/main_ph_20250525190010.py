...
def run_proofing_phase(glossary, log_message):
    proofreader = Proofreader(log_message, glossary.get_current_glossary_file())
    ...
    for fname in os.listdir("output"):
        ...
        updated_lines = proofreader.detect_and_fix_non_english(lines, glossary_text=glossary_text)

        if updated_lines and updated_lines != lines:
            with open(file_path, "w", encoding="utf-8") as f:
                f.writelines(updated_lines)
            log_message(f"[FIXED] Non-English content retranslated and updated in: {fname}")
...
def main(log_message=None, glossary_file=None, proofing_only=False,
    ...
    run_proofing_phase(glossary, log_message)
