...
def run_proofing_phase(glossary, log_message, pause_event=None, cancel_flag=None):
    ...
    flagged_lines = proofreader.detect_and_log_non_english_sentences(
        "output",
        non_english_log,
        "input",
        pause_event,
    ...
    if flagged_lines:
        joined = "\n".join(flagged_lines)
        log_message(f"[NON-ENGLISH] {len(flagged_lines)} non-English lines detected. Sending as chunked logs...")

        encoded = joined.encode("utf-8")
        chunk_size = 5120  # 5 KB
        for i in range(0, len(encoded), chunk_size):
            chunk = encoded[i:i+chunk_size].decode("utf-8", errors="ignore")
            log_message(f"[NON-ENGLISH] {chunk}")

        # 🔁 Rerun fix on each affected file
        from proofing.glossary_utils import load_proofing_glossaries
        ...
        for line in flagged_lines:
            ...
            try:
                ...
                glossary_text, _ = load_proofing_glossaries(glossary.get_current_glossary_file(), log_message)
                fixed_lines = proofreader.detect_and_fix_non_english(lines, glossary_text=glossary_text)

                if fixed_lines and fixed_lines != lines:
                    with open(fpath, "w", encoding="utf-8") as f:
                        f.writelines(fixed_lines)

                    # Print diff line-by-line
                    for idx, (orig, updated) in enumerate(zip(lines, fixed_lines)):
                        if orig != updated:
                            log_message(f"[NON-ENGLISH] {fname} (line {idx+1}):")
                            log_message(f"  ⛔ {orig.strip()}")
                            log_message(f"  ✅ {updated.strip()}")

                    log_message(f"[NON-ENGLISH] Updated lines in {fname}")
                else:
                    log_message(f"[NON-ENGLISH] No updates needed in {fname}")
            except Exception as e:
                log_message(f"[ERROR] Failed to retranslate {fname}: {e}")
    else:
        log_message("[NON-ENGLISH] No untranslated content detected.")
    ...
    for fname in translated_files:
        try:
            ...
            log_message(f"[ERROR] Proofing failed for {fname}: {e}")
