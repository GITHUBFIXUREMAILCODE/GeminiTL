...
def proof_glossary_file(glossary_path: str, log_message=print, max_retries=3, retry_delay=60):
    """
    Splits large glossaries and proofreads them with Gemini in 10KB chunks,
    allowing cross-batch context with each iteration.
    """
    ...
    for i, chunk in enumerate(glossary_chunks):
        prompt = "\n".join(PROOF_GLOSSARY_INSTRUCTIONS) + f"\n\nGlossary Part {i+1}:\n" + chunk
        attempt = 0
