...
class Glossary:
    ...
    def build_glossary(self, input_text, log_message, glossary_filename=None,
        ...
        "The text may contain mature themes, but all characters and situations are entirely fictional.",
        ...
        "Honorifics (e.g., -san, -chan, -sama, -kun, -sensei, -dono, -senpai) should be preserved exactly as-is in romanized form when connected to a original term.",
        ...
        "Use the exact format: Original RAW Term => English Translation => Gender Pronoun",
        ...
        "some examples include: バーディ => Birdie => he/him, 白魔道服 => White Magic Robe => it/its, とりまる様 => Torimaru-sama => they/them, 土人形 => Earth Figure => it/its, ゴーレム => Golem => it/its"
        ]
    ...
    def clean_glossary(self, log_message=None):
        """
        Clean the current glossary by removing English terms and symbols.
        
        Args:
            log_message: Optional callback for logging messages
            
        Returns:
            bool: True if cleaning was successful, False otherwise
        """
        if log_message is None:
            log_message = self.log_message or print
            
        if not self.current_glossary_path or not os.path.exists(self.current_glossary_path):
            log_message("[ERROR] No glossary file to clean")
            return False
            
        try:
            from glossary.glossary_cleaner import clean_glossary
            return clean_glossary(self.current_glossary_path, log_message)
        except Exception as e:
